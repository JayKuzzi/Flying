...
public class OrderDFragment extends MyFragment {
    private View view;
    ...
    private String waitDoneOrderNum;//要完成的订单编号
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case Constant.QUERY_LIST_SUCCESS:
                    queryDialog.dismiss();
                    Toast.makeText(getActivity(), "取货单 查询成功", Toast.LENGTH_SHORT).show();
                ...
                case Constant.SERVER_NOT_RETURN_FOR_UD:
                    mRecyclerAdapterInD.getDialog.dismiss();
                    Toast.makeText(getActivity(), "服务器无返回 请检查异常", Toast.LENGTH_SHORT).show();
                    break;
                case Constant.UPDATE_ORDER_SUCCESS:
                    mRecyclerAdapterInD.getDialog.dismiss();
                    refresh();
                    Toast.makeText(getActivity(), "成功取货 您的订单已完成", Toast.LENGTH_SHORT).show();
                    sendNotifyToUser();
                    break;
                case Constant.UPDATE_ORDER_FAIL:
                    mRecyclerAdapterInD.getDialog.dismiss();
                    Toast.makeText(getActivity(), "取货失败 请检查服务器", Toast.LENGTH_SHORT).show();
                    break;
            }
        }
    };
    ...
    private void initData() {
        ...
        queryDialog.setMessage("从数据库读取货单 请稍后");
    ...
    private void initView_RecyclerView() {
        ...
        mRecyclerAdapterInD.setOnListChangedListener(new OrderRecyclerAdapterInD.ListChangedListener() {
            @Override
            public void onListChangedClick(String doneOrderNum) {
                if (!checkNetwork()) {
                    Toast toast = Toast.makeText(getActivity(), "网络未连接哦", Toast.LENGTH_SHORT);
                    toast.setGravity(Gravity.CENTER, 0, 0);
                    toast.show();
    ...
    protected void loadData() {
        Toast.makeText(getActivity(), "4可见 正在懒加载数据", Toast.LENGTH_SHORT).show();
        initData();
    }
    ...
    private class queryOrderThread implements Runnable {
        @Override
        public void run() {
            String state = "%E5%BE%85%E5%8F%96";//表示浏览器编码--"待取"
    ...
    private class updateOrderThread implements Runnable {
        ...
        public void run() {
            ...
            try {
                object.put("waitDoneOrderNum", waitDoneOrderNum);
